...
class HierarchicalReasoningModel_ACTV1_Inner_Trace(HierarchicalReasoningModel_ACTV1_Inner):
    ...
    def forward(
        ...
        # All steps without grad, except the very last z_L and z_H update
        with torch.no_grad():
        ...
        z_L_logit_lens = self.lm_head(z_L)[:, self.puzzle_emb_len:]

        trace.record_step(z_H, z_L, output, z_L_logit_lens)

        return new_carry, output, (q_logits[..., 0], q_logits[..., 1])


class HierarchicalReasoningModel_ACTV1_Trace(HierarchicalReasoningModel_ACTV1):

    def __init__(self, config_dict: dict):
        super().__init__(config_dict)
        self.inner = HierarchicalReasoningModel_ACTV1_Inner_Trace(self.config)
