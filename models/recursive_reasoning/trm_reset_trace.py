...
from models.recursive_reasoning.trm_reset import (
    ...
    def forward(
        ...
        with torch.no_grad():
            for _H_step in range(self.config.H_cycles-1):
                if self.config.reset_z_H_per_H_cycle:
                    z_H = self.H_init.expand_as(z_H)
                if self.config.reset_z_L_per_H_cycle:
                    z_L = self.L_init.expand_as(z_L)
